  ...
  return (
    <div>
      <div className="mb-2 d-flex justify-content-between align-items-center custom-board-detail-title">
        <div className="ms-1" style={{ fontSize: "18px" }}>
          {board.title}
        </div>
      ...
      <div className="d-flex justify-content-between align-items-center ms-1">
        <div>
          <span className="ms-3 me-3" style={{ color: "rgb(128,128,128)" }}>
            {board.user?.username}
          </span>
        </div>
        <div className="me-2">
          <span className="me-1" style={{ color: "rgb(128,128,128)" }}>
            조회수: {board.views}
          </span>
        </div>
      </div>
      ...
      {isAuthenticated && userId === board.user?.userId && board.editable && (
        <div className="d-flex justify-content-end mb-2">
          <Link
            to={`/board/updateForm/${board.boardId}`}
      ...
      <div className="ms-2 py-3" style={{ whiteSpace: "pre" }}>
        {board.content}
      </div>
      <hr />
    </div>
